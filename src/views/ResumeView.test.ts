...
describe('ResumeView', () => {
  it('shows research voyage, education, skills and English certificates', () => {
    ...
    expect(text).not.toContain('海丝远洋科考经历')
    expect(text).not.toContain('厦门大学第六届“海丝学堂”人才培养计划')
    expect(text).not.toContain('嘉庚号第二航段组员')
    ...
    expect(wrapper.find('[data-to="/projects/haisi"]').exists()).toBe(false)
    expect(wrapper.find('.voyage-detail-link').exists()).toBe(false)
